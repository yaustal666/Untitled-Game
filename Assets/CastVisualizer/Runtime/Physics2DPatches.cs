...
namespace CCG.CastVisualizer
{
    ...
    public sealed class Physics2DVisualizer : BaseVisulizer<Physics2DMeshShape, Collider2D>
    {
        ...
        partial class PhysicsScene2DPatches
        {
...
#if UNITY_2023_1_OR_NEWER
            // extern private static int LinecastList_Internal(PhysicsScene2D physicsScene, Vector2 start, Vector2 end, ContactFilter2D contactFilter, [NotNull("ArgumentNullException")] List<RaycastHit2D> results)
            [HarmonyPrefix]
            [HarmonyPatch("LinecastList_Internal")]
            static void LinecastList_Internal_Prefix(Vector2 start, Vector2 end)
            {
                if (CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_1))   // Line/Raycasts
                {
                    Vector2 dir = end - start;
            ...
            [HarmonyPostfix]
            [HarmonyPatch("LinecastList_Internal")]
            static void LinecastList_Internal_Postfix(List<RaycastHit2D> results)
            {
                if (CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_1))   // Line/Raycasts
                {
                    foreach (RaycastHit2D hit in results)
        ...
        partial class PhysicsScene2DPatches
        {
            ...
            static void CircleCast_Internal_Postfix(RaycastHit2D __result, Vector2 origin, float radius, Vector2 direction, float distance)
            {
                if (CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_4))   // CircleCasts
                {
                    ...
                    AddMeshToRender((Physics2DMeshShape.Circle, Matrix4x4.TRS(origin, Quaternion.identity, size), direction * safeDistance * 2.0f));
                }
            }
        ...
        partial class PhysicsScene2DPatches
        {
            ...
            [HarmonyPrefix]
            [HarmonyPatch("OverlapColliderFilteredList_Internal")]
            static void OverlapColliderFilteredList_Internal_Prefix(Collider2D collider)
            {
            ...
            [HarmonyPostfix]
            [HarmonyPatch("OverlapColliderFilteredList_Internal")]
            static void OverlapColliderFilteredList_Internal_Postfix(List<Collider2D> results, Collider2D collider)
            {
            ...
            [HarmonyPrefix]
            [HarmonyPatch("OverlapColliderFilteredArray_Internal")]
            static void OverlapColliderFilteredArray_Internal_Prefix(Collider2D collider)
            {
                if (CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_8))   // OverlapColliders
                {
                    AddColliderCastToRender(collider);
            ...
            [HarmonyPostfix]
            [HarmonyPatch("OverlapColliderFilteredArray_Internal")]
            static void OverlapColliderFilteredArray_Internal_Postfix(Collider2D[] results, Collider2D collider)
            {
                foreach (Collider2D coll in results)
                {
                    if (coll != null && CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_8))   // OverlapColliders
            ...
            // ToDo: Add support for position and angle
            // extern private static int OverlapColliderFromList_Internal(Vector2 position, float angle, [NotNull("ArgumentNullException")] Collider2D collider, [NotNull("ArgumentNullException")] List<Collider2D> results)
            [HarmonyPrefix]
            [HarmonyPatch("OverlapColliderFromList_Internal")]
            static void OverlapColliderFromList_Internal_Prefix(Vector2 position, float angle, Collider2D collider)
            {
                if (CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_8))   // OverlapColliders
                {
                    AddColliderCastToRender(collider);
            ...
            [HarmonyPostfix]
            [HarmonyPatch("OverlapColliderFromList_Internal")]
            static void OverlapColliderFromList_Internal_Postfix(List<Collider2D> results)
            {
                foreach (Collider2D coll in results)
                {
                    if (coll != null && CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_8))   // OverlapColliders
            ...
            // extern private static int OverlapColliderFromFilteredList_Internal(Vector2 position, float angle, [NotNull("ArgumentNullException")] Collider2D collider, ContactFilter2D contactFilter, [NotNull("ArgumentNullException")] List<Collider2D> results)
            [HarmonyPrefix]
            [HarmonyPatch("OverlapColliderFromFilteredList_Internal")]
            static void OverlapColliderFromFilteredList_Internal_Prefix(Vector2 position, float angle, Collider2D collider)
            {
                if (CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_8))   // OverlapColliders
                {
                    AddColliderCastToRender(collider);
            ...
            [HarmonyPostfix]
            [HarmonyPatch("OverlapColliderFromFilteredList_Internal")]
            static void OverlapColliderFromFilteredList_Internal_Postfix(List<Collider2D> results)
            {
                foreach (Collider2D coll in results)
                {
                    if (coll != null && CastVisualizerManager.Instance.ViewStatePhysics2DFlag.HasFlag(CastVisualizerManager.ViewStateFlags.ViewStateElement_8))   // OverlapColliders
